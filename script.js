...
// VERSÃO COM FILTRO DE PROMINÊNCIA PARA RESTAURANTES
// ========================================================================
...
// --- Serviços de Rota (Inicializados, mas rota ainda desativada) ---
...
// --- Variáveis não usadas nesta fase de teste ---
// const markers = [];
...
function updateUserMarkerAndAccuracy(position) {
    ...
    try {
        if (userLocationMarker) { // Atualiza existente
            ...
            if (userLocationMarker.getMap() !== map) { console.warn(">>> updateUserMarkerAndAccuracy: SETA readicionada ao mapa."); userLocationMarker.setMap(map); }
...
function initMap() {
    ...
    userLocationMarker = null; userLocationAccuracyCircle = null; // Garante reset
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => { // ERRO INICIAL
                ...
                currentUserLocation = null; const defaultCoords = { lat: -23.5505, lng: -46.6333 };
                initializeMapAndServices(defaultCoords, 13);
                handleLocationError(error, false);
            },
            { enableHighAccuracy: true, timeout: 10000, maximumAge: 60000 }
        );
    } else { /* ... sem geolocalização ... */ }
}
...
function startWatchingPosition() {
     ...
     if (watchId !== null) { console.warn(">>> startWatchingPosition: Limpando watchId anterior:", watchId); try { navigator.geolocation.clearWatch(watchId); } catch(e){} watchId = null; }
     console.log(">>> startWatchingPosition: Tentando iniciar...");
     try {
         watchId = navigator.geolocation.watchPosition(
             (newPosition) => { console.log("--- watchPosition: Sucesso. Chamando update..."); updateUserMarkerAndAccuracy(newPosition); },
             (error) => { console.error("!!! watchPosition: ERRO recebido:", error.code, error.message); handleLocationError(error, true); },
...
/**
 * Inicializa o mapa, Serviços (Places, Directions) e chama outras inicializações.
 */
function initializeMapAndServices(initialCoords, initialZoom) {
    ...
    try {
        ...
        directionsService = new google.maps.DirectionsService(); // Reativado
        directionsRenderer = new google.maps.DirectionsRenderer({ map: map, suppressMarkers: false }); // Reativado
        console.log(">>> initializeMapAndServices: Directions criados.");
        console.log(">>> initializeMapAndServices: Serviços Google prontos.");

        setupEventListeners(); // Chama setup dos listeners

        // Chama update inicial da seta e inicia watcher DEPOIS que tudo está pronto
    ...
    } catch (error) { console.error("!!! ERRO GERAL em initializeMapAndServices:", error); /* ... */ }
}
...
function setupEventListeners() {
    ...
    let missingElement = null; /* ... verificações ... */
    ...
    categoryButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (!map || !placesService || typeof placesService.nearbySearch !== 'function') { /* ... erro ... */ return; }
            console.log(`--- Iniciando busca para "${categoryType}" ---`);
            if(routeFoundBtn) routeFoundBtn.disabled = true;
            clearFoundMarkers();

            // ***** INÍCIO DA MODIFICAÇÃO DO FILTRO *****
            let request;
            if (currentUserLocation) {
                // TEM localização do usuário, usa nearbySearch
                if (categoryType === 'restaurant') { // <<< CONDIÇÃO PARA RESTAURANTE
                    // Se for restaurante, usa PROMINENCE
                    console.log("--- Buscando RESTAURANTES PRÓXIMOS por PROMINÊNCIA ---");
                    request = {
                    ...
                    // Se for outra categoria (incluindo bar), usa RADIUS
                ...
                console.log(`--- Buscando ${categoryType} na área visível (textSearch) ---`);
...
function handleSearchResults(results, status) {
    ...
    // const routeFoundBtn = document.getElementById('route-found-btn'); // Já é global

    clearFoundMarkers(); // Limpa antes de adicionar novos
    ...
    if (status === google.maps.places.PlacesServiceStatus.OK && results && results.length > 0) {
        ...
        results.forEach((place, index) => {
            if (place.name && place.geometry && place.geometry.location) {
                ...
                try {
                     const marker = new google.maps.Marker({ position: place.geometry.location, map: map, title: place.name }); // Marcador padrão
        ...
        } else { console.warn(">>> handleSearchResults: Status OK, mas nenhum marcador válido."); alert("Nenhum local com localização encontrado."); if (routeFoundBtn) routeFoundBtn.disabled = true; }
    } else if (status === google.maps.places.PlacesServiceStatus.ZERO_RESULTS) {
        console.log(">>> handleSearchResults: ZERO_RESULTS."); alert("Nenhum local encontrado."); if (routeFoundBtn) routeFoundBtn.disabled = true;
    } else { console.error(`!!! handleSearchResults: Erro na busca! Status: "${status}"`); alert(`Erro ao buscar locais: ${status}.`); if (routeFoundBtn) routeFoundBtn.disabled = true; }
